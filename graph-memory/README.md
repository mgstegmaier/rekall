...
- `build_index.py --corpus ~/obsidian-vault/heck-db/wiki` chunks every wiki page on headings (h1–h3, packed to ~1600 chars), prefixes each chunk's index text with the page's title and description (the deterministic form of contextual retrieval; the stored excerpt stays the raw section), embeds it locally with BGE-small (fastembed, CPU, no API; `[index] embedding_model` in `rekall.toml` can name another fastembed model, which then needs `--full`), and writes keyword (FTS5) + vector indexes to `rag.db` next to the script. Runs are incremental by default (a content hash per file in the `files` table; only new/changed/deleted files are touched, seconds on a typical day). `--full` drops and rebuilds everything (~24 min for the whole wiki) — the first run each Sunday promotes itself to full automatically (`last_full` date in the `meta` table, so a Mac asleep at the scheduled hour still gets its weekly full after waking), and `--full` stays required after an embedding-model change since stored vectors from the old model are invalid. A DB without the `files` table (pre-incremental) upgrades itself with one automatic full build.
...
`eval/` measures the hook: `sample_hits.py` freezes a set of real prompts from Claude Code transcripts and records what each leg returns, `judge.py` labels every returned item with a headless Haiku call and scores precision per leg, position, and predicate (pass two label dirs to average two judge passes; one pass wobbles about 5 points). Every retrieval change since 2026-09-12 was decided on it; numbers and decisions live in `docs/plans/2026-09-12-typed-wiki-graph.md` and `docs/plans/2026-09-13-contextual-retrieval.md`. A model-written context sentence per chunk (`contextualize.py`) was tried and measured flat at 300 prompts, then removed; the deterministic title/description prefix in `build_index.py` stays.
